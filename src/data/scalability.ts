...
export interface ScalabilityItem {
  title: string
  body: string
  tag?: string           // optional badge: 'use-cases' | 'jurisdictions' | 'people'
  tagColor?: string      // accent color key e.g. 'blue' | 'amber' | 'purple'
}
...
export const scalabilitySections: Record<string, ScalabilitySection> = {
  ...
  overview: {
    tabId: 'overview',
    framing: 'This reference architecture is scoped to one entity, one jurisdiction, and one workflow. Each of those constraints is a deliberate simplification — and each creates a specific structural problem when removed.',
    ...
    items: [
      ...
      },
      {
        ...
        body: 'The four governance layers — Authority, Accountability, Change, Data & Decision — remain the correct analytical frame at any scale. The V1 architecture is not wrong; it is bounded. Understanding exactly where and why it breaks is the precondition for designing a scalable successor.',
      },
    ],
    crossLinks: [
